...
def test_ecbsr():
    ...
    net = ECBSR(num_in_ch=1, num_out_ch=1, num_block=16, num_channel=64, with_idt=False, act_type='prelu', scale=4).cuda()
    img = torch.rand((1, 1, 256, 256), dtype=torch.float32).cuda()
    print(flop_count_table(FlopCountAnalysis(net, img), activations=ActivationCountAnalysis(net, img)))
    output = net(img)
