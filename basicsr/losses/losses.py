...
from basicsr.utils.freq_util import get_high_freq
from basicsr.utils.registry import LOSS_REGISTRY
from .loss_util import weighted_loss, GaussianFilter
...
class GaussianLoss(nn.Module):
    def __init__(self, kernel_size=21, loss_weight=1.0, reduction='mean'):
        super(GaussianLoss, self).__init__()
        # self.kernel_size = kernel_size
        self.loss_weight = loss_weight
        self.lowpass_filter = GaussianFilter(kernel_size)
        self.criterion = torch.nn.L1Loss(reduction=reduction)

    def forward(self, pred, target):
        pred = pred - self.lowpass_filter(pred)
        target = target - self.lowpass_filter(target)
